...
bool sortInput(istream& infile, StudentType students[], int& size){
	...
	while (true){
		...
		if (temp.grade < 0 || temp.grade > 100) {			//This line gets rid of invalid data -- numbers that are out of range
			cout << "Infile contains invalid data." << endl;
			return false;
		}
		if (infile.eof()) break;							//Files have to end with a return as the last character 
...
//displayList
//Lists student data
//Assumption: names are sorted and array of student data is filled. 
...
void setHistogram(StudentType dataToSet[], int* histogram, const int& size) {
	...
	for (int gradeIndex = 0; gradeIndex < size; gradeIndex++){ 
		...
		currentGrade = currentGrade / GROUP		;
...
// Assumption: histogram data is filled. 
...
//Assumption: The array of students is filled. 
